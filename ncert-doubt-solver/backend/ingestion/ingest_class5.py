...
DATA_ROOT = Path(__file__).resolve().parents[2] / 'data' / 'class_5'


def ingest_class5(pdfs_root: Path = None):
    ...
    # iterate subject folders hu 
    for subject_dir in sorted(pdfs_root.iterdir()):
        if not subject_dir.is_dir():
            continue
        if subject_dir.name == 'vector_db' or subject_dir.name.startswith('.'):
            continue
